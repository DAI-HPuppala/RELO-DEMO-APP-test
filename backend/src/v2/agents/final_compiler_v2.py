...
class FinalCompilerV2:
    ...
    def __init__(self):
        ...
        logger.info("FinalCompilerV2 initialized for aggregation")
    ...
    async def compile_results(self, agent_results: Dict[str, Dict[str, Any]]) -> FinalClassification:
        ...
        # Extract attributes from each agent
        if "initial_classifier" in agent_results:
            initial = agent_results["initial_classifier"].get("attributes", {})
        ...
        if "detail_extractor" in agent_results:
            detail = agent_results["detail_extractor"].get("attributes", {})
        ...
        if "damage_detector" in agent_results:
            damage = agent_results["damage_detector"].get("attributes", {})

...
class VLMGPULoader:
    ...
    def __init__(self, ollama_host: str = None, model_name: str = None):
        ...
        # Performance metrics - use deque for O(1) automatic eviction
        ...
        try:
            ...
            async with self.optimized_inference():
                ...
                if agent_name == "detail_extractor":
                    ...
                    top_p = 0.4       # Very conservative sampling
                    top_k = 5       # Only best choice for OCR accuracy
                    ...
                    top_k = 15        # More choices to recognize diverse damage types
                    ...
                    temperature = 0.4  # Slightly more than damage for variety
                ...
                if agent_name == "initial_classifier":
                    num_predict = 250  # Increased from 150 to prevent attribute truncation
                elif agent_name == "detail_extractor":
                    num_predict = 180  # Increased from 100 to ensure full brand/size extraction
                elif agent_name == "damage_detector":
                    num_predict = 350  # Increased from 200 for complete bbox + damage descriptions
                else:
                    num_predict = 400  # Increased from 300 for safer default
                ...
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.post(f"{self.ollama_host}/api/generate", json=payload) as response:
                        if response.status == 200:
                            ...
                            # Try to parse JSON response with fallback for truncated responses
                            try:
                                if raw_response:
            ...
            logger.error(f"Optimized inference failed: {e}", exc_info=True)
            return {
                "error": str(e),
                "inference_metadata": {

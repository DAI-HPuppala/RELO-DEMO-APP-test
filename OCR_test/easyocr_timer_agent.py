#!/usr/bin/env python3
"""
Timer-Based GPU OCR Agent with 1-Second Time Budget
Intelligent OCR agent that attempts multiple parameter combinations within time limit
"""
...
class RobustGPUOCRAgent:
    ...
    def __init__(self, time_budget: float = 1.0):
        self.vlm_dir = Path("/home/denaliai/RELO-CLASSIFIER-DEV/RELO-DEMO-APP-test-RELO-DEV-APP-test/vlm_optimization_experiments")
        self.output_dir = Path("/home/denaliai/RELO-CLASSIFIER-DEV/RELO-DEMO-APP-test-RELO-DEV-APP-test/text_zoom_experiments/timer_agent_results")
        self.output_dir.mkdir(exist_ok=True)

        # Time budget for each image (default 1 second)
        ...
        print("="*70)
        print("TIMER-BASED GPU OCR AGENT (1-Second Budget)")
        print("="*70)
        ...
        print(f"⏱️  Time Budget: {self.time_budget}s per image")
    ...
    def _order_combinations_by_brightness(self, brightness: float) -> List[Dict]:
        """Order parameter combinations based on image brightness"""
        combos = self.param_combinations.copy()

        if brightness > 128:
            # Bright image: prioritize bilateral and standard methods
            # Good for dark text on light background
            priority_order = ['default_bilateral', 'sensitive_bilateral', 'very_sensitive_standard', 'sensitive_unsharp']
        else:
            # Dark image: prioritize unsharp and sensitive methods
            # Good for light text on dark background
            priority_order = ['sensitive_unsharp', 'very_sensitive_standard', 'sensitive_bilateral', 'default_bilateral']
    ...
    def multi_inference(self, image_path: Path) -> Dict:
        """
        Perform multiple inference attempts within time budget using optimal parameter combinations
        """
        ...
        # Reorder parameter combinations based on brightness
        ordered_combos = self._order_combinations_by_brightness(brightness)
        ...
        # Attempt combinations within time budget
        for combo in ordered_combos:
            # Check time budget BEFORE starting attempt
            elapsed = time.time() - time_start
            if elapsed >= self.time_budget:
                break

            # Estimate if we have enough time for another attempt (min 50ms needed)
            ...
            if result['confidence'] > 0.9 and result['text']:
                break
    ...
    def run_agent(self):
        ...
        print("="*70)
        print(f"Processing {len(image_files)} images with {self.time_budget}s time budget per image")
        print(f"Will attempt multiple parameter combinations within time limit")
        print(f"Results will be saved to JSON")
        print("="*70 + "\n")
        ...
        # Calculate actual total attempts (not fixed at 4 per image for timer version)
        actual_total_attempts = sum(r['attempts'] for r in all_image_results)
        ...
        comprehensive_results = {
            ...
            'summary': {
                ...
                'total_attempts': actual_total_attempts,  # Actual attempts made within time budget
                'avg_attempts_per_image': round(actual_total_attempts / total_images, 1) if total_images > 0 else 0
            },
        ...
        # Save to JSON file
        json_path = self.output_dir / f"timer_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
if __name__ == "__main__":
    # Initialize agent with 1-second time budget
    agent = RobustGPUOCRAgent(time_budget=1.0)

    # Run the timer-based agent
